...
const VisuallyHidden = ({
  ...
  <span
    className={cn(
      "absolute w-px h-px p-0 m-[-1px] overflow-hidden clip-[rect(0,0,0,0)] whitespace-nowrap border-0",
      className
    )}
    {...props}
  />
...
>(({ className, children, disableOutsideClick = false, ...props }, ref) => {
  return (
    <DialogPortal>
      <DialogOverlay 
        className={disableOutsideClick ? "cursor-default" : "cursor-pointer"}
      />
      <DialogPrimitive.Content
        ...
        onPointerDownOutside={(e) => disableOutsideClick && e.preventDefault()}
      ...
      >
        ...
        <DialogClose className="absolute cursor-pointer right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground">
          <X className="h-4 w-4 dark:text-white/75 text-black" />
