...
import { Users, Search, Filter, MoreHorizontal, UserCheck, Eye, Edit, Ban, Download, Plus } from "lucide-react";
...
export default function ActiveUsers() {
  ...
  // Pre-filter for active users only
  ...
  const handleExportUsers = async () => {
    ...
    try {
      ...
      console.log('Exporting active users...');
    } catch (error) {
  ...
  const stats = users.list ? [
    { label: "Total Active", value: users.list.length, icon: UserCheck, color: "bg-green-500" },
    { label: "Online Now", value: users.list.filter(u => u.lastLoginAt && new Date(u.lastLoginAt) > new Date(Date.now() - 5 * 60 * 1000)).length, icon: Users, color: "bg-blue-500" },
    { label: "KYC Verified", value: users.list.filter(u => u.kyc?.verificationStatus === "Approved").length, icon: Users, color: "bg-purple-500" },
    { label: "Email Verified", value: users.list.filter(u => u.emailVerified).length, icon: Users, color: "bg-orange-500" },
  ] : [];
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">Users with active status only.</p>
          </div>
