...
import { Clock, CheckCircle, XCircle, Eye, DollarSign, Search, Download, Plus } from "lucide-react";
import { AppDispatch, RootState } from "@/store";
import { fetchManualDeposits, approveDeposit, rejectDeposit } from "@/store/slices/adminSlice";
...
export default function PendingDeposits() {
  ...
  // Pre-filter for pending deposits only
  ...
  const handleApproveConfirm = async (comment?: string) => {
    if (selectedDeposit) {
      try {
        await dispatch(approveDeposit({ id: selectedDeposit.id, comment }));
  ...
  const handleRejectConfirm = async (rejectionReason: string) => {
    if (selectedDeposit) {
      try {
        await dispatch(rejectDeposit({ id: selectedDeposit.id, rejectionReason }));
        // Refresh the deposits list
        dispatch(fetchManualDeposits({
          page: currentPage,
          limit: pageSize,
  ...
  const handleExportDeposits = async () => {
    ...
    try {
      ...
      console.log('Exporting pending deposits...');
    } catch (error) {
  ...
  const stats = deposits.list ? [
    { label: "Total Pending", value: deposits.list.length, icon: Clock, color: "bg-yellow-500" },
    { label: "Total Amount", value: `$${deposits.list.reduce((sum, deposit) => sum + deposit.amount, 0).toLocaleString()}`, icon: DollarSign, color: "bg-green-500" },
    { label: "Avg. Amount", value: `$${deposits.list.length > 0 ? (deposits.list.reduce((sum, deposit) => sum + deposit.amount, 0) / deposits.list.length).toFixed(2) : '0'}`, icon: DollarSign, color: "bg-blue-500" },
    { label: "With Proof", value: deposits.list.filter(d => d.proofFileUrl).length, icon: Eye, color: "bg-purple-500" },
  ] : [];
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">Manual deposits awaiting approval.</p>
          </div>
        ...
        <ApproveDialog
          ...
          message={`Are you sure you want to approve this deposit of $${selectedDeposit?.amount?.toLocaleString()}?`}
        />
        ...
        <RejectDialog
          ...
          message={`Are you sure you want to reject this deposit of $${selectedDeposit?.amount?.toLocaleString()}?`}
        />
